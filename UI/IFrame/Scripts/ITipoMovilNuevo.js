...
function init(data) {
    ...
    cargarResultadoConsulta(tipos);

    //------------------------------------
    // Click
    //------------------------------------

    $("#btn_agregarTipo").click(
        function () {
            insertar()
        });
...
function insertar() {
    ...
    if (obs == "" || obs == undefined) {
        mostrarMensaje('Error', 'Debe ingresar el tipo de móvil');
        return;
    }
    ...
    crearAjax({
        Data: { comando: data },
        Url: ResolveUrl('~/Servicios/TipoMovilService.asmx/Insert'),
        OnSuccess: function (result) {
            ...
            mostrarMensaje('Exito', "El tipo de móvil '"+result.Return.Nombre+"' se ha creado correctamente");
            agregarRow(result.Return);
            $("#inputFormulario_Nombre").val('');
            $("#inputFormulario_Nombre").trigger('focus');
        },
        OnError: function (result) {
            mostrarMensaje('Error', 'Error insertando el tipo de móvil');
            mostrarMensaje('Error', result.Error);
        }
    });
}
...
function initTablaResultadoConsulta() {
    $('#tablaTiposMovil').DataTableGeneral({
        Orden:[[0,'asc']],
        Columnas: [
            {
                title: 'Fecha Alta',
        ...
        Botones: [
            {
                ...
                OnClick: function (data) {
                    var obs = data.Nombre;
                    crearDialogoHTML({
                        Titulo: '<label>Editar Tipo de móvil</label>',
                        Content: '<div class="row margin-top" >' +
                                            '<div class="col s12">' +
                                                '<div class="input-field">' +
                                                    '<input id="inputFormulario_NombreEditar" type="text"/>' +
                                                    '<label for="inputFormulario_NombreEditar" class=" no-select textarea">Tipo de móvil</label>' +
                                                '</div>' +
                                            '</div>' +
                                        '</div>',
                                ...
                                },
                                {
                                    ...
                                    OnClick: function (jAlert) {
                                        ...
                                        crearAjax({
                                            ...
                                            Url: ResolveUrl('~/Servicios/TipoMovilService.asmx/Editar'),
                                            OnSuccess: function (result) {
                                                //Oculto el cargando
                                                ...
                                                mostrarMensaje('Exito', 'Tipo móvil editado correctamente.');
                                            ...
                                            error: function (result) {
                                                ...
                                                //Muestro el Error
                                                mostrarMensaje('Error', 'Error editando el tipo de móvil');
                                                console.log(result);
                                            }
                                        });
                                    }
                        ...
                        CallbackEditar: function (tipo) {
                            actualizarRowEnGrilla(tipo);
                        }
                    });
                }
            },

            {
                ...
                OnClick: function (data) {
                    ...
                    crearDialogoHTML({
                        Titulo: '<label>Eliminar Tipo de móvil</label>',
                        Content:
                            '<div class="padding">' +
                            '<label id="textoConfirmacion"class="titulo">¿Está seguro de eliminar el tipo de móvil?</label>' +
                            '</div>',
                        Botones:
                            [
                                {
                                ...
                                },
                                {
                                    ...
                                    OnClick: function (jAlert) {
                                        ...
                                        var urlAjax = ResolveUrl('~/Servicios/TipoMovilService.asmx/DarDeBaja');
...
function calcularCantidadDeRows() {
    ...
    var dt = $('#tablaTiposMovil').DataTable();
