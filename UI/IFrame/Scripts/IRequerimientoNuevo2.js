...
function init(data) {
    ...
    SelectorMotivo_SetOnMensajeListener(function (tipo, mensaje) {
        mostrarMensaje(tipo, mensaje);
    });

    SelectorMotivo_SetOnMotivoSeleccionadoListener(function (motivo) {
    ...
    SelectorMotivo_SetOnModoCambiadoListener(function (modoManual) {
        //Oculto el error
        $('#errorFormulario_Motivo').stop(true, true).hide(300);
    });
    ...
    $('#btn_SeleccionarDomicilio').click(function () {
        $('#errorFormulario_Domicilio').stop(true, true).slideUp(300);

        crearDialogoUbicacionSelector({
            Callback: function (domicilio) {
                domicilioSeleccionado = domicilio;

                $('#contenedor_DomicilioNoSeleccionado').stop(true, true).fadeOut(300, function () {
                    $('#contenedor_DomicilioSeleccionado').stop(true, true).fadeIn(300);
                    if (domicilio.Direccion != undefined) {
                        $('#texto_DomicilioTitulo').html('<b>Dirección' + (domicilio.Sugerido ? ' aprox' : '') + ': </b>' + toTitleCase(domicilio.Direccion) + (domicilio.Distancia > 0 ? ' (' + domicilio.Distancia + ' m)' : '') + '</b></label>');
                    } else {
                        $('#texto_DomicilioTitulo').text('');
                    }

                    if (domicilio.Observaciones != undefined) {
                        $('#texto_DomicilioDescripcion').html('<b>Descripción: </b>' + domicilio.Observaciones);
                    } else {
                        $('#texto_DomicilioDescripcion').html('<b>Descripción: </b> Sin datos');
                    }

                    if (domicilio.Barrio != undefined) {
                        $('#texto_DomicilioBarrio').html('<b>Barrio: </b>' + toTitleCase(domicilio.Barrio.Nombre));
                    } else {
                        $('#texto_DomicilioBarrio').html('<b>Barrio: </b>Sin datos');
                    }

                    if (domicilio.Cpc != undefined) {
                        $('#texto_DomicilioCpc').html('<b>Cpc: </b>N° ' + domicilio.Cpc.Numero + ' - ' + toTitleCase(domicilio.Cpc.Nombre));
                    } else {
                        $('#texto_DomicilioCpc').html('<b>Cpc: </b>Sin datos');
                    }

                    let url = 'https://maps.googleapis.com/maps/api/staticmap?autoscale=false&size=300x300&maptype=roadmap&key=""&format=png&visual_refresh=true&markers=size:mid%7Ccolor:0xff0000%7Clabel:%7C' + domicilio.Latitud.replace(',', '.') + ',' + domicilio.Longitud.replace(',', '.');
                    $('#contenedor_DomicilioSeleccionado .mapa').css('background-image', 'url(' + url + ')');
                });
            }
        });
    });
    ...
    $('#btn_CancelarDomicilio').click(function () {
        ...
        $('#contenedor_DomicilioSeleccionado').stop(true, true).fadeOut(300, function () {
            $('#contenedor_DomicilioNoSeleccionado').stop(true, true).fadeIn(300);
        });
    });
...
function getRequerimiento() {
    ...
    rq.RelevamientoInterno = $('#check_RelevamientoInterno').is(':checked');
    ...
    if (domicilioSeleccionado != undefined) {
        var comandoUbicacionNuevo = {
            ...
            Direccion: domicilioSeleccionado.Sugerido ? undefined : domicilioSeleccionado.Direccion
        };
