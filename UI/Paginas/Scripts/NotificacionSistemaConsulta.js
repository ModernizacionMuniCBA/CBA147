...
function init(data) {
    ...
    cargarResultadoConsulta(notificaciones);
    ...
    $('#btnNuevo').click(function () {
        crearDialogoNotificacionParaUsuarioNuevo({
            CallbackMensajes: function (tipo, mensaje) {
                mostrarMensaje(tipo, mensaje);
            },
            Callback: function (entity) {
                notificaciones.push(entity);
                filtrarBusqueda();
            }
        });
    });
...
function filtrarBusqueda() {
    ...
    $.each(notificaciones, function (index, entity) {

        if (entity.Titulo.toUpperCase().indexOf(texto) != -1) {
...
function initTablaResultadoConsulta() {
    tabla = $('#tabla').DataTableNotificacionParaUsuario({
        Orden: [[1, 'asc'], [2, 'asc']],
        CallbackMensajes: function(tipo, mensaje){
            mostrarMensaje(tipo, mensaje);
        },
        ...
        BotonDarDeBaja: true,
        BotonDarDeAlta: true,
        BotonSubirPosicion: true,
        BotonBajarPosicion: true,
        BotonNotificar: true,
        BotonNoNotificar: true,
        CallbackEditar: function(entityNueva){
            actualizarRowEnGrilla(entityNueva);
            tabla.sort().draw(false);
        },
        CallbackDarDeBaja: function (entityNueva) {
            actualizarRowEnGrilla(entityNueva);
        },
        CallbackDarDeAlta: function (entityNueva) {
            actualizarRowEnGrilla(entityNueva);
        },
        CallbackNotificar: function (entityNueva) {
            actualizarRowEnGrilla(entityNueva);
        },
        CallbackNoNotificar: function (entityNueva) {
            actualizarRowEnGrilla(entityNueva);
        }
    });
...
function actualizarRowEnGrilla(entity) {
    ...
    $.each(notificaciones, function (index, element) {
        if (element.Id == entity.Id) {
            notificaciones[index] = entity;
            return;
        }
    });
}
