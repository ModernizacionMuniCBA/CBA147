...
function initTabla() {
    $.each(infoPermisos, function (index, element) {
        element.EstadoNombre = $.grep(estados, function (element1, index1) {
            return element1.KeyValue == element.EstadoRequerimiento;
        })[0].Nombre;
    ...
    $.each(estados, function (index, estado) {
        cols.push({
            ...
            render: function (data, type, row, meta) {
                ...
                $.each(infoPermisos, function (index, element) {
                    if (element.Permiso == permiso.KeyValue && element.EstadoRequerimiento == estado.KeyValue) {
...
function initBotones() {
    $('#btn_Guardar').click(function () {
        ...
        crearAjax({
            Url: ResolveUrl('~/Servicios/PermisoEstadoRequerimientoService.asmx/SetPermisos'),
            ...
            OnSuccess: function (result) {
                ...
                crearAjax({
                    Url: ResolveUrl('~/Servicios/PermisoEstadoRequerimientoService.asmx/GetPermisos'),
