...
function initConsulta() {
    ...
    estados.forEach(function (estado) {
        ...
        if (jQuery.inArray(estado.Id, arrayKeyValue) !== -1) {
...
function initResultado() {
    ...
    $('#btnGenerarExcel').click(function () {
        ...
        setTimeout(function () {
            crearDialogoReporteEstadisticaUsuario(getBase64, datosService.consulta, getTextoFiltrosConsulta());
        }, t);
    });
}
...
function consultar2() {
    ...
    var url = ResolveUrl('~/Servicios/EstadisticaService.asmx/GetDatosEstadisticaUsuario');
...
function generarGrafico(estadisticas, torta) {
    ...
    $.each(estadisticas, function (index, estadistica) {
        if (estadistica.Cantidad > 0) {
            data.push({
                label: toTitleCase(estadistica.Usuario),
                value: estadistica.Cantidad,
                toolText: "<div id='nameDiv'>" + toTitleCase(estadistica.Usuario) + "</div>{br}Cantidad: <b>" + estadistica.Cantidad + "</b>{br}Porcentaje: <b>" + estadistica.Porcentaje + "%</b>",
                link: 'j-clickGrafico-' + estadistica.IdUsuario
            });
        }
    });
    ...
    $.each(estadisticas, function (index, estadistica) {
        if (estadistica.Cantidad > 0) {
            data_grilla.push({
                nombre: toTitleCase(estadistica.Usuario),
...
function clickGrafico(idUsuario) {
    $.each(data_grafico, function (index, data) {
        if (data.IdUsuario == idUsuario) {
            mostrarDetalle(data.Usuario, data.IdsRequerimientos);
        }
    });
}
...
function mostrarDetalle(usuario, ids) {
    var objeto = 'Requerimientos del Usuario ';

    var titulo = objeto + toTitleCase(usuario);
