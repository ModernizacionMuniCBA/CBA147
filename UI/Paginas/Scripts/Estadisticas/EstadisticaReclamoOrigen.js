...
function initResultado() {
    ...
    $('#btnGenerarExcel').click(function () {
        ...
        var getBase64 = obtenerBase64Grafico();
        setTimeout(function () {
            crearDialogoReporteEstadisticaOrigen(getBase64, datosService.consulta, getTextoFiltrosConsulta());
        }, t);
    });
}
...
function consultar2() {
    ...
    var url = ResolveUrl('~/Servicios/EstadisticaService.asmx/GetDatosEstadisticaOrigen');
...
function generarGrafico(estadisticas, torta) {
    ...
    $.each(estadisticas, function (index, estadistica) {
        if (estadistica.Cantidad > 0) {
            data.push({
                label: toTitleCase(estadistica.Origen),
                value: estadistica.Cantidad,
                toolText: "<div id='nameDiv'>" + toTitleCase(estadistica.Origen) + "</div>{br}Cantidad: <b>" + estadistica.Cantidad + "</b>{br}Porcentaje: <b>" + estadistica.Porcentaje + "%</b>",
                link: 'j-clickGrafico-' + estadistica.IdOrigen
            });
        }
    });
    ...
    $.each(estadisticas, function (index, estadistica) {
        if (estadistica.Cantidad > 0) {
            data_grilla.push({
                nombre: toTitleCase(estadistica.Origen),
...
function clickGrafico(idOrigen) {
    $.each(data_grafico, function (index, data) {
        if (data.IdOrigen == idOrigen) {
            mostrarDetalle(data.Origen, data.IdsRequerimientos);
        }
    });
}
...
function mostrarDetalle(origen, ids) {
    var objeto = 'Requerimientos';

    var titulo = objeto + ' del Origen ' + toTitleCase(origen);
