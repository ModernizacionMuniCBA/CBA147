...
function initResultado() {
    ...
    $('#btnGenerarExcel').click(function () {
        ...
        setTimeout(function () {
            crearDialogoReporteEstadisticaRubros(getBase64, datosService.consulta, getTextoFiltrosConsulta());
        }, t);
    });
}
...
function getFiltrosConsulta() {
    ...
    //Rubros del Grupo seleccionado
    var g = $('#selectFormulario_Grupo').val();
    idGrupo = parseInt(g);
    filtros.idGrupo = idGrupo;

    //var rubrosFiltradas = _.where(rubros, { GrupoRubroId: idGrupo })
    //filtros.Rubros = rubrosFiltradas;
...
function getTextoFiltrosConsulta() {
    ...
    $.each(getFiltrosConsulta(), function (key, val) {
        switch (key) {
            ...
            case 'idGrupo': {
                var idGrupo = parseInt($('#selectFormulario_Grupo').val());
                key = 'Grupo';

                var grupoString = _.where(grupos, { Id: idGrupo })
                val = grupoString[0].Nombre;
            } break;
        }
...
function consultar2() {
    ...
    var url = ResolveUrl('~/Servicios/EstadisticaService.asmx/GetDatosEstadisticaRubros');
...
function generarGrafico(estadisticas, torta) {
    ...
    $.each(estadisticas, function (index, estadistica) {
        if (estadistica.Cantidad > 0) {
            data.push({
                label: toTitleCase(estadistica.Rubro),
                value: estadistica.Cantidad,
                toolText: "<div id='nameDiv'>" + toTitleCase(estadistica.Rubro) + "</div>{br}Cantidad: <b>" + estadistica.Cantidad + "</b>{br}Porcentaje: <b>" + estadistica.Porcentaje + "%</b>",
                link: 'j-clickGrafico-' + estadistica.IdRubro
            });
        }
    });
    ...
    $.each(estadisticas, function (index, estadistica) {
        if (estadistica.Cantidad > 0) {
            data_grilla.push({
                nombre: toTitleCase(estadistica.Rubro),
...
function clickGrafico(idRubro) {
    $.each(data_grafico, function (index, data) {
        if (data.IdRubro == idRubro) {
            mostrarDetalle(data.Rubro, data.IdsRequerimientos);
        }
    });
}
...
function mostrarDetalle(rubro, ids) {
    var objeto = 'Requerimientos';

    var titulo = objeto + ' del Rubro ' + toTitleCase(rubro);
