...
function init(data) {
    ...
    setTimeout(function () {
        ControlSelectorRangoFecha_OcultarPeriodos();
    }, 500);

    setTimeout(function () {
        ControlSelectorRangoFecha_SetAñoMes(año, mes);
        setTimeout(function () {
            consultar2();
        }, 500);
    }, 500);
...
function initResultado() {
    ...
    $('#btnGenerarExcel').click(function () {
        ...
        setTimeout(function () {
            crearDialogoReporteEstadisticaResueltos(getBase64, datosService.consulta, getTextoFiltrosConsulta());
        }, t);
    });
}
...
function consultar2() {
    ...
    var url = ResolveUrl('~/Servicios/EstadisticaService.asmx/GetDatosEstadisticaResueltos');
...
function generarGrafico(estadisticas, torta) {
    ...
    $.each(estadisticas, function (index, estadistica) {
        if (estadistica.Cantidad > 0) {
            data.push({
                label: toTitleCase(estadistica.Etiqueta),
                value: estadistica.Cantidad,
                toolText: "<div id='nameDiv'>" + toTitleCase(estadistica.Etiqueta) + "</div>{br}Cantidad: <b>" + estadistica.Cantidad + "</b>{br}Porcentaje: <b>" + estadistica.Porcentaje + "%</b>",
                link: 'j-clickGrafico-' + estadistica.Etiqueta
            });
        }
    });
    ...
    $.each(estadisticas, function (index, estadistica) {
        if (estadistica.Cantidad > 0) {
            data_grilla.push({
                nombre: toTitleCase(estadistica.Etiqueta),
...
function clickGrafico(etiqueta) {
    $.each(data_grafico, function (index, data) {
        if (data.Etiqueta == etiqueta) {
            mostrarDetalle(etiqueta, data.IdsRequerimientos);
        }
    });
}
...
function mostrarDetalle(etiqueta, ids) {
    var objeto = 'Requerimientos de ';

    var titulo = objeto + toTitleCase(etiqueta);
