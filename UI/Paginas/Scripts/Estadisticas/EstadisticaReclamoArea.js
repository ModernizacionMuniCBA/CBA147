...
function initResultado() {
    ...
    $('#btnGenerarExcel').click(function () {
        ...
        setTimeout(function () {
            crearDialogoReporteEstadisticaArea(getBase64, datosService.consulta, getTextoFiltrosConsulta());
        }, t);
    });
}
...
function validarConsulta() {
    ...
    if ($('#select_Servicio').val() == -1) {
        $('#select_Servicio').siblings('.control-observacion').text('Dato requerido');
        $('#select_Servicio').siblings('.control-observacion').stop(true, true).show(300);
        valido = false;
    }
...
function getFiltrosConsulta() {
    ...
    var valueServicio = $('#select_Servicio').val();
    filtros.IdServicio = valueServicio;
    ...
    $.each(getFiltrosConsulta(), function (key, val) {
        switch (key) {
            ...
            case 'IdServicio': {
                ...
                val = toTitleCase($.grep(servicios, function (e) { return e.Id == val; })[0].Nombre);
            } break;
...
function consultar2() {
    ...
    var url = ResolveUrl('~/Servicios/EstadisticaService.asmx/GetDatosEstadisticaArea');
