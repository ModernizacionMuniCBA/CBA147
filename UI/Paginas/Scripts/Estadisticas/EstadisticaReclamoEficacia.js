...
function initResultado() {
    ...
    $('#btnGenerarExcel').click(function () {
        ...
        setTimeout(function () {
            crearDialogoReporteEstadisticaEficacia(getBase64, datosService.consulta, getTextoFiltrosConsulta());
        }, t);
    });
}
...
function consultar2() {
    ...
    var url = ResolveUrl('~/Servicios/EstadisticaService.asmx/GetDatosEstadisticaEficacia');
...
function generarGrafico(estadisticas, torta) {
    ...
    $.each(estadisticas, function (index, estadistica) {
        if (estadistica.Cantidad > 0) {
            data.push({
                label: toTitleCase(estadistica.Condicion),
                value: estadistica.Cantidad,
                toolText: "<div id='nameDiv'>" + toTitleCase(estadistica.Condicion) + "</div>{br}Cantidad: <b>" + estadistica.Cantidad + "</b>{br}Porcentaje: <b>" + estadistica.Porcentaje + "%</b>",
                link: 'j-clickGrafico-' + estadistica.Condicion
            });
        }
    });
    ...
    $.each(estadisticas, function (index, estadistica) {
        if (estadistica.Cantidad > 0) {
            data_grilla.push({
                nombre: toTitleCase(estadistica.Condicion),
...
function clickGrafico(condicion) {
    $.each(data_grafico, function (index, data) {
        if (data.Condicion == condicion) {
            mostrarDetalle(data.Condicion, data.IdsRequerimientos);
        }
    });
}
...
function mostrarDetalle(condicion, ids) {
    var objeto = 'Requerimientos';

    var titulo = objeto + toTitleCase(condicion);
