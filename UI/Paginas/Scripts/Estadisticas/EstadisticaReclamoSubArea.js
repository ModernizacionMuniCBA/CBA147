...
function initConsulta() {
    ...
    if (areas != undefined) {
        ...
        /*Solo voy a cargar areas que tengan subareas porque la estadistica es de subareas*/
        var soloAreasConSubarea = [];

        areas.forEach(function (areaSel) {
            if (areaSel.Subareas != undefined) {
                soloAreasConSubarea.push(areaSel);
            }
        });

        //Cargo los datos
        if (soloAreasConSubarea != undefined) {
            $('#select_Area').CargarSelect({
                Data: soloAreasConSubarea,
                Value: 'Id',
                Text: 'Nombre',
                Default: 'Seleccione..',
                Sort: true
...
function initResultado() {
    ...
    $('#btnGenerarExcel').click(function () {
        ...
        setTimeout(function () {
            crearDialogoReporteEstadisticaSubArea(getBase64, datosService.consulta, getTextoFiltrosConsulta());
        }, t);
    });
}
...
function getFiltrosConsulta() {
    ...
    var areaSel = _.where(areas, { Id: parseInt(valueArea) })

    var arrSub = [];
    subarray = areaSel[0].Subareas;

    subarray.forEach(function (sub) {
        arrSub.push(sub.Id)
    });
    
    filtros.idsAreas = arrSub;
    ...
    $.each(getFiltrosConsulta(), function (key, val) {
        switch (key) {
            ...
            case 'idsAreas': {
                ...
                var areasString = "";
                $.each(subarray, function (index, subarea) {
                    if (areasString != "") {
                        areasString += ', ';
                    }
                    areasString += subarea.Nombre;
                });
                val = areasString;
            } break;
            }
...
function consultar2() {
    ...
    var url = ResolveUrl('~/Servicios/EstadisticaService.asmx/GetDatosEstadisticaSubArea');
...
function generarGrafico(estadisticas, torta) {
    ...
    $.each(estadisticas, function (index, estadistica) {
        if (estadistica.Cantidad > 0) {
            data.push({
                label: toTitleCase(estadistica.Area),
                value: estadistica.Cantidad,
                toolText: "<div id='nameDiv'>" + toTitleCase(estadistica.Area) + "</div>{br}Cantidad: <b>" + estadistica.Cantidad + "</b>{br}Porcentaje: <b>" + estadistica.Porcentaje + "%</b>",
                link: 'j-clickGrafico-' + estadistica.IdArea
            });
        }
    });
    ...
    $.each(estadisticas, function (index, estadistica) {
        if (estadistica.Cantidad > 0) {
            data_grilla.push({
                nombre: toTitleCase(estadistica.Area),
...
function clickGrafico(idArea) {
    if (idArea != 0) {
        $.each(data_grafico, function (index, data) {
            if (data.IdArea == idArea) {
                mostrarDetalle(data.Area, data.IdsRequerimientos);
            }
        });
    } else {
        mostrarMensaje('Alerta', 'No se permite ver los requerimientos de otras áreas');
    }
...
function mostrarDetalle(area, ids) {
    var objeto = 'Requerimientos del Area ';

    var titulo = objeto + toTitleCase(area);
