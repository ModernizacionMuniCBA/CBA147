...
function initJs() {
    ...
    try {
        ...
        $.fn.dataTableExt.oStdClasses.sPageButton =
          "btn-flat btn-paginacion waves-effect";
        $.fn.dataTableExt.oStdClasses.sPageButtonActive =
          "grey white-text waves-light";
...
function initForms() {
    ...
    try {
        ...
        var inputs = $(
          ".input-field > input, .input-field > textarea, .input-field > select, .mi-input-field > select"
        );
        ...
        jQuery.extend(jQuery.validator.messages, {
            ...
            rangelength: jQuery.validator.format(
              "Ingrese entre {0} y {1} caracteres."
            ),
...
jQuery.fn.CargarSelect = function (valores) {
    ...
    $(this)
      ...
      .on("select2:open", function () {
          ...
          var xAnterior = parseInt(
            window.top.$(".select2-container--open").css("left"),
            10
          );
          var xNuevo = xAnterior + xOffset;

          var yAnterior = parseInt(
            window.top.$(".select2-container--open").css("top"),
            10
          );
...
jQuery.fn.AgregarCheckbox = function (valores) {
    ...
    $("<label />", {
        for: "cb" + valores.Value,
        color: "black",
        text: valores.Name,
        width: "180px",
        id: "cblb" + valores.Value
    }).appendTo($(this));
};
...
jQuery.fn.AgregarIndicadorCargando = function (valores) {
    ...
    var spinner =
      ' <div class="preloader-wrapper big active"><div class="spinner-layer"><div class="circle-clipper left"><div class="circle"></div></div><div class="gap-patch"><div class="circle"></div></div><div class="circle-clipper right"><div class="circle"></div></div></div></div>';
    var divCargando =
      "<div id='indicadorCargando' class='contenedor' style='position:absolute; top:0; left:0; margin:0; padding:0; width:100%; height:100%'><div class='cargando " +
      clases +
      "'>" +
      spinner +
      "</div></div>";
...
jQuery.fn.DataTableReclamo2 = function (valores) {
            ...
            render: function (data, type, row) {
                ...
                switch (data) {
                    case 1:
                        {
                            prioridad =
                              '<i class="material-icons indicador activo colorTextoPrioridadNormal tooltipped" data-position="bottom" data-delay="50" data-tooltip="Prioridad baja">flag</i>';
                        }
                        break;
                    ...
                    case 2:
                        {
                            prioridad =
                              '<i class="material-icons indicador activo colorTextoPrioridadMedia tooltipped" data-position="bottom" data-delay="50" data-tooltip="Prioridad media">flag</i>';
                        }
                        break;
                    ...
                    case 3:
                        {
                            prioridad =
                              '<i class="material-icons indicador activo colorTextoPrioridadAlta tooltipped" data-position="bottom" data-delay="50" data-tooltip="Prioridad alta">flag</i>';
                        }
                        break;
                }
                ...
                tooltip =
                  row.OrdenEspecialId == null
                    ? "Sin Orden de Atención Crítica"
                    : "Orden de Atención Crítica " +
                      toTitleCase(row.EstadoOrdenEspecialNombre);
                var ordenEspecial =
                  '<div><i class="material-icons tooltipped indicador' +
                  apagado +
                  '" data-position="bottom" style="color: #' +
                  row.EstadoOrdenEspecialColor +
                ...
                tooltip =
                  row.Marcado == false
                    ? "En control de Área Operativa"
                    : "En control de CPC";
                var marcado =
                  '<div class="flex"><label class="tooltipped indicador' +
                  apagado +
                  '"  data-position="bottom" data-delay="50" data-tooltip="' +
                  tooltip +
                  '" style="border-radius: 25px; background: #9F25F6; color: white;padding: 0.2rem;font-weight: bolder; font-size: 0.5rem; line-height: 1rem;">CPC</label></div>';

                ////Inspeccionado
                //apagado = row.Inspeccionado == false ? "" : claseActivo;
                //tooltip =
                //  row.Inspeccionado == false ? "Sin inspección" : "Inspeccionado";
                //var inspeccionado =
                //  '<div><i style="padding-left: 3px" class="material-icons tooltipped indicador indicador-inspeccionado' +
                //  apagado +
                //  '" data-position="bottom"  data-delay="50" data-tooltip="' +
                //  tooltip +
                //  '">done_all</i>';
                ...
                if (row.Favorito != undefined) {
                    ...
                    favorito =
                      '<div class="btnToggleFavorito no-select indicador indicador-favorito ' +
                      apagado +
                      '" id-rq="' +
                      row.Id +
                      '"><i class="material-icons tooltipped" data-position="bottom" data-delay="50" data-tooltip="' +
                      tooltip +
                      '">star</i>';
                }

                return (
                  '<div><label style="display:none;">a</label>' +
                  estado +
                  prioridad +
                  urgente +
                  ordenEspecial +
    ...
    $("#" + idTabla).on("click", ".btnToggleFavorito", function () {
        ...
        var url = ResolveUrl(
          "~/Servicios/RequerimientoService.asmx/ToggleFavorito"
        );
    ...
    cols.push(
      {
          ...
          render: function (data, type, row) {
              return (
                '<div><span class="link-numero tooltipped" data-position="bottom" data-delay="50" data-tooltip="Ver detalle" id-rq="' +
                row.Id +
                '">' +
                row.Numero +
                "/" +
      ...
      },
      {
          ...
          render: function (data, type, row) {
              ...
              return (
                "<div><span>" + fecha.format("DD/MM/YYYY HH:mm") + "</span></div>"
              );
          }
      },
      {
    ...
    //Boton unirse a Rq
    if (!("BotonUnirseARequerimiento" in valores)) {
        valores.BotonUnirseARequerimiento = false;
    }

    if (!("CallbackUnirseARequerimiento" in valores)) {
        valores.CallbackUnirseARequerimiento = function () { };
    }

    if (valores.BotonUnirseARequerimiento) {
        botones.push({
            Titulo: "Unirse a Requerimiento",
            Icono: "exposure_plus_1",
            OnClick: function (data) {
                crearDialogoConfirmacion({
                    Texto:
                      "¿Está seguro que desea unirse al requerimiento? Su descripción y toda la información que sugirió, se agregará al mismo.",
                    TextoBotonAceptar: "Aceptar",
                    CallbackPositivo: function () {
                        valores.CallbackUnirseARequerimiento(data.Id);
                    }
                });
            }
        });
    ...
    function convertirResultadoTabla(id, callback, callbackError) {
        crearAjax({
            Url: ResolveUrl(
              "~/Servicios/RequerimientoService.asmx/GetResultadoTablaById"
            ),
            Data: { id: id },
            OnSuccess: function (result) {
                if (!result.Ok) {
...
jQuery.fn.DataTableOrdenTrabajo = function (valores) {
    ...
    //Columna de ambito
    if (!("ColumnaAmbito" in valores)) {
        valores.ColumnaAmbito = false;
    }
    ...
    cols.push(
      {
          ...
          render: function (data, type, row) {
              return (
                '<div><span class="link-numero tooltipped" data-position="bottom" data-delay="50" data-tooltip="Ver detalle" id-ot="' +
                row.Id +
                '">' +
                row.Numero +
                "</span></div>"
    ...
    function convertirResultadoTabla(id) {
        return new Promise(function (callback, callbackError) {
            crearAjax({
                Url: ResolveUrl(
                  "~/Servicios/OrdenTrabajoService.asmx/GetDatosTablaByIds"
                ),
                Data: { ids: [id] },
                OnSuccess: function (result) {
                    if (!result.Ok) {
...
jQuery.fn.DataTableOrdenInspeccion = function (valores) {
    ...
    function convertirResultadoTabla(id) {
        return new Promise(function (callback, callbackError) {
            crearAjax({
                Url: ResolveUrl(
                  "~/Servicios/OrdenInspeccionService.asmx/GetDatosTablaByIds"
                ),
                Data: { ids: [id] },
                OnSuccess: function (result) {
                    if (!result.Ok) {
...
jQuery.fn.DataTableOrdenAtencionCritica = function (valores) {
    ...
    var cols = [
      {
          ...
          render: function (data, type, row) {
              return (
                ...
                '" data-position="bottom" data-delay="50" data-tooltip="' +
                toTitleCase(data) +
                '">security</i> <span style="margin-left: 8px">' +
                toTitleCase(data.EstadoNombre) +
                "</span></div>"
              );
          }
      },
      ...
      },
      {
          ...
          render: function (data, type, row) {
              return (
                '<div><span class="tooltipped"  data-position="bottom" data-delay="50" data-tooltip="' +
                toTitleCase(data) +
                '">' +
                toTitleCase(data) +
                "</span></div>"
    ...
    function convertirResultadoTabla(rq, callback, callbackError) {
        crearAjax({
            Url: ResolveUrl(
              "~/Servicios/OrdenAtencionCriticaService.asmx/GetResultadoTablaById"
            ),
            Data: { id: rq.Id },
            OnSuccess: function (result) {
                if (!result.Ok) {
...
jQuery.fn.DataTablePersonaFisica = function (valores) {
    ...
    botones.push({
        ...
        OnClick: function (data) {
            ...
            if (creado == false) {
                valores.CallbackMensajes(
                  tipo,
                  "Error creando el dialogo del detalle de la persona física"
                );
            }
        }
    });
    ...
    botones.push({
        ...
        OnClick: function (data) {
            ...
            if (creado == false) {
                valores.CallbackMensajes(
                  tipo,
                  "Error creando el dialogo de la edición de la persona física"
                );
            }
        }
    });
...
jQuery.fn.DataTableMovil = function (valores) {
    ...
    //Boton Checkbox
    if (!("BotonCheckbox" in valores)) {
        valores.BotonCheckbox = false;
    }
    ...
    if (!"ColumnaDominio" in valores) {
        valores.ColumnaDominio = true;
    }

    //Boton Borrar
    if (!("BotonBorrar" in valores)) {
        valores.BotonBorrar = false;
    }

    //Callback Borrar
    if (!("CallbackBorrar" in valores)) {
        valores.CallbackBorrar = function () { };
    }

    var cols = [];
    if (valores.BotonCheckbox) {
        cols.push({
            title: "",
            data: "Id",
            width: "24px",
            render: function (data, type, full, meta) {
                return (
                  '<div><p style="margin:0px; padding:0px; margin-top:6px;"><input type="checkbox" value="' +
                  data +
                  '" id="chx' +
                  data +
                  '" ><label style="  padding:0px!important" for="chx' +
                  data +
                  '"></label> </p></div>'
                );
            }
    ...
    cols.push(
      {
          ...
          render: function (data, type, row) {
              return (
                "<div><span class='link-detalle' id-entity='" +
                row.Id +
                "'>" +
                toTitleCase(data) +
                "</span></div>"
      ...
      },
      {
          ...
          render: function (data, type, row) {
              return (
                "<div><span>" +
                toTitleCase(row.Marca + " " + row.Modelo) +
                "</span></div>"
              );
          }
      }
    ...
    if (valores.BotonBorrar) {
        //Boton para borrar el empleado
        botones.push({
            Icono: "delete",
            Texto: "Quitar",
            OnClick: function (data) {
                valores.CallbackBorrar(data);
            }
        });
    }
    ...
    if (valores.BotonSeleccionar) {
        ...
        botones.push({
            ...
            Visible: !dadoDeBaja,
            OnClick: function (data) {
                valores.CallbackSeleccionar(data.Id);
            }
        });
    }
...
jQuery.fn.DataTableCampoPorMotivo = function (valores) {
    var idTabla = "" + $(this).prop("id");
    if (valores == undefined) valores = {};
    ...
    valores.Definiciones = [{ type: "num-html", targets: 0 }];
    ...
    var botones = [
      {
          ...
          Visible: function (data, type, row) {
              if (data.FechaBaja == null) return true;
              return false;
          },
          OnClick: function (data) {
              var obs = 1;
              crearDialogoCampoPorMotivoEditar({
                  Id: data.Id,
                  Callback: function (c) {
                      actualizarEnTabla(c);
                  }
...

      {
          ...
          OnClick: function (data) {
              crearDialogoHTML({
                  Titulo: "<label>Eliminar Campo por motivo</label>",
                  Content:
                    '<div class="padding">' +
                    '<label id="textoConfirmacion"class="titulo">¿Está seguro de eliminar el campo por motivo?</label>' +
                    "</div>",
                  Botones: [
                    {
                        Id: "btnNo",
                    ...
                    },
                    {
                        ...
                        OnClick: function (jAlert) {
                            ...
                            var urlAjax = ResolveUrl(
                              "~/Servicios/MotivoService.asmx/EliminarCampo"
                            );
                            ...
                            crearAjax({
                                ...
                                OnSuccess: function (result) {
                                    ...
                                    mostrarMensaje("Exito", "Tipo eliminado correctamente");

                                    //Cierro el dialogo
                                    $(jAlert).CerrarDialogo();
                                    actualizarEnTabla(result.Return);
                                },
                                OnError: function (result) {
                                    //Oculto el cargando
                                    mostrarCargando(false);
                                    ...
                                    //Muestro el Error
                                    mostrarMensaje("Error", "Error eliminando el tipo");
                                }
                            });
                        }
                    }
    ...
    function actualizarEnTabla(entity) {
        ...
        //Busco el indice del campo a actualizar
        var index = -1;
        var dt = $("#" + idTabla).DataTable();
        dt.rows(function (idx, data, node) {
            if (data.Id == entity.Id) {
...
//    return procesarDatatable(idTabla, valores);
//}
jQuery.fn.DataTableFlota = function (valores) {
    var idTabla = "" + $(this).prop("id");
    if (valores == undefined) valores = {};
    ...
    cols.push(
      ...
      {
          title: "NombreSinAcentos",
          data: "Nombre",
          visible: false,
          render: function (data, type, row) {
              var r = (data + " " + row.Apellido).toLowerCase();
              ...
              return "<div><span class='link-detalle' >" + r + "</span></div>";
          }
      },
      {
          title: "Móvil",
          data: "Movil.NumeroInterno",
          render: function (data, type, row) {
              return "<div><span>" + data + "</span></div>";
          }
      },
          ...
          render: function (data, type, row) {
              if (
                ...
                data.length == 0
                  ...
                  return "<div><span>Sin empleados</span></div>";
              }
              ...
              $.each(data, function (index, emp) {
                  ...
                  primera = false;
                  empleados += emp.Nombre + ' ' + emp.Apellido;
              });

              return "<div><span>" + empleados + "</span></div>";
          }
      }
    );
    ...
    function convertirResultadoTabla(id) {
        return new Promise(function (callback, callbackError) {
            crearAjax({
                Url: ResolveUrl(
                  "~/Servicios/FlotaService.asmx/GetResultadoById"
                ),
                Data: { id: id },
                OnSuccess: function (result) {
                    if (!result.Ok) {
...
jQuery.fn.DataTableEmpleado = function (valores) {
    var idTabla = "" + $(this).prop("id");
    if (valores == undefined) valores = {};
    ...
    // Funciones
    //--------------------------------------
    if (!("Funciones" in valores)) {
        valores.Funciones = function () {
            return [];
        };
    }
    ...
    cols.push(
      ...
      },
      {
          ...
          data: "NombreSeccion",
          render: function (data, type, row) {
              if (data != null) return "<div><span>" + data + "</span></div>";
          }
      },
      {
          title: "Funciones",
          ...
          render: function (data, type, row) {
              if (
                data == null ||
                data.length == 0 ||
                valores.Funciones().length == 0
              ) {
                  return "<div><span>Sin funciones</span></div>";
              }

              var funciones = "";
              var primera = true;
              $.each(data, function (index, id) {
                  var funcion = $.map(valores.Funciones(), function (obj) {
                  ...
                  primera = false;
                  funciones += funcion[0].Nombre;
              });

              return "<div><span>" + funciones + "</span></div>";
          }
      }
    );
    ...
    function convertirResultadoTabla(id) {
        return new Promise(function (callback, callbackError) {
            crearAjax({
                Url: ResolveUrl(
                  "~/Servicios/EmpleadoService.asmx/GetResultadoTablaById"
                ),
                Data: { id: id },
                OnSuccess: function (result) {
                    if (!result.Ok) {
    ...
    botones.push({
        ...
        OnClick: function (data) {
            ...
            if (creado == false) {
                valores.CallbackMensajes(
                  tipo,
                  "Error creando el dialogo del detalle del origen"
                );
            }
        }
    });
    ...
    botones.push({
        ...
        OnClick: function (data) {
            ...
            if (creado == false) {
                valores.CallbackMensajes(
                  tipo,
                  "Error creando el dialogo de la edición del origen"
                );
            }
        }
    });
...
jQuery.fn.DataTableSeccion = function (valores) {
    ...
    var cols = [
      {
          ...
          render: function (data, type, row) {
              return (
                '<div class="link-detalle" id-entity="' +
                row.Id +
                '"><span>' +
                toTitleCase(data) +
                "</span></div>"
    ...
    function verDetalle(id) {
        ...
        if (creado == false) {
            valores.CallbackMensajes(
              tipo,
              "Error creando el dialogo de la edición del sección"
            );
        }
    }
...
jQuery.fn.DataTableRubroMotivo = function (valores) {
    var idTabla = "" + $(this).prop("id");
    ...
    function verDetalle(id) {
        var creado = crearDialogoRubroMotivoEditar({
            Id: id,
            CallbackMensajes: function (tipo, mensaje) {
                valores.CallbackMensajes(tipo, mensaje);
            },
            Callback: function (c) {
                actualizarRubroMotivoEnGrilla(c);
                valores.Callback(c);
            }
        });
        ...
        if (creado == false) {
            valores.CallbackMensajes(
              tipo,
              "Error creando el dialogo de la edición de la categoría"
            );
        }
    }

    function actualizarRubroMotivoEnGrilla(cat) {
        ...
        if (dataVieja.IdGrupo != cat.IdGrupo || cat.FechaBaja != null) {
            var dt2 = $("#" + idTabla).dataTable();
            dt2.fnDeleteRow(index);
            return;
        }
...
jQuery.fn.DataTableTareas = function (valores) {
    var idTabla = "" + $(this).prop("id");
    ...
    //Callback Actualizar
    if (!("CallbackActualizar" in valores)) {
        valores.CallbackActualizar = function () { };
    }
    ...
    function verDetalle(id) {
        var creado = crearDialogoTareaDetalle({
            Id: id,
            CallbackMensajes: function (tipo, mensaje) {
                valores.CallbackMensajes(tipo, mensaje);
            },
            Callback: function () {
                getResultadoTabla(
                  id,
                  function (tarea) {
                      actualizarTareaEnGrilla(tarea);
                      valores.CallbackActualizar(tarea);
                  },
                  function () {
                      valores.CallbackMensajes(
                        "Error",
                        "Error actualizando la tarea en la tabla"
                      );
                  }
                );
            }
        ...
        if (creado == false) {
            valores.CallbackMensajes(
              tipo,
              "Error creando el dialogo de detalle de la tarea"
            );
        }
    }

    function actualizarTareaEnGrilla(t) {
        //Busco el indice de la persona a actualizar
        var index = -1;
        var dt = $("#" + idTabla).DataTable();
        var dataVieja;
    ...
    function getResultadoTabla(id, callback, callbackError) {
        crearAjax({
            Url: ResolveUrl("~/Servicios/TareaService.asmx/GetResultadoTablaById"),
            Data: { id: id },
            OnSuccess: function (result) {
                if (!result.Ok) {
                    if (callbackError != undefined) {
...
jQuery.fn.DataTableEdificioMunicipal = function (valores) {
    var idTabla = "" + $(this).prop("id");
    ...
    function verDetalle(id) {
        var creado = crearDialogoEdificioMunicipalEditar({
            Id: id,
            CallbackMensajes: function (tipo, mensaje) {
                valores.CallbackMensajes(tipo, mensaje);
            },
            Callback: function (e) {
                valores.CallbackEditar(e);
            }
        });
        ...
        if (creado == false) {
            valores.CallbackMensajes(
              tipo,
              "Error creando el dialogo de detalle del edificio municipal"
            );
        }
    }
    ...
    function actualizarEdificioEnGrilla(e) {
        //Busco el indice de la persona a actualizar
        var index = -1;
        var dt = $("#" + idTabla).DataTable();
        var dataVieja;
    ...
    function getResultadoTabla(id, callback, callbackError) {
        crearAjax({
            Url: ResolveUrl(
              "~/Servicios/EdificioMunicipalService.asmx/GetResultadoTablaById"
            ),
            Data: { id: id },
            OnSuccess: function (result) {
                if (!result.Ok) {
...
jQuery.fn.DataTableUsuarioATI = function (valores) {
    var idTabla = "" + $(this).prop("id");
    ...
    cols.push(
      ...
      {
          title: "Área Territorial de Incumbencia",
          data: "ATINombre",
          render: function (data, type, row) {
              return "<div><span>" + toTitleCase(data) + "</span></div>";
          }
    ...
    function verDetalle(id) {
        var creado = crearDialogoUsuarioATINuevo({
            Id: id,
            CallbackMensajes: function (tipo, mensaje) {
                valores.CallbackMensajes(tipo, mensaje);
            },
        ...
        if (creado == false) {
            valores.CallbackMensajes(
              tipo,
              "Error creando el dialogo de detalle del usuario asociado a un ATI"
            );
        }
    }
    ...
    function actualizarUsuarioATIEnGrilla(e) {
        //Busco el indice de la persona a actualizar
        var index = -1;
        var dt = $("#" + idTabla).DataTable();
        var dataVieja;
...
jQuery.fn.DataTableUsuario = function (valores) {
    ...
    //Observaciones
    //--------------------------------------
    if (!("ColumnaObservaciones" in valores)) {
        valores.ColumnaObservaciones = false;
    }
              ...
              if (row.IdentificadorFotoPersonal != undefined) {
                  ...
                  if (row.SexoMasculino == true) {
                      foto =
                        top.urlCordobaFiles +
                        "/Archivo/" +
                        top.identificadorFotoUserMale +
                        "/3";
                  } else {
                      foto =
                        top.urlCordobaFiles +
                        "/Archivo/" +
                        top.identificadorFotoUserFemale +
                        "/3";
                  }
              }
              return (
                '<div><div class="foto" style="min-width: 2rem; min-height:2rem; background-image: url(' +
                foto +
                ');border-radius: 2rem; max-width: 2rem;background-size: cover;background-repeat: no-repeat;max-height: 2rem;"></div></div>'
              );
          }
      },
      {
    ...
    if (valores.ColumnaObservaciones) {
        cols.push({
            title: "Observaciones",
            data: "Observaciones",
            render: function (data, type, row) {
    ...
    botones.push({
        ...
        OnClick: function (data) {
            ...
            if (creado == false) {
                valores.CallbackMensajes(
                  tipo,
                  "Error creando el dialogo del detalle del usuario"
                );
            }
        }
    });
    ...
    botones.push({
        Titulo: "Borrar referente",
        Icono: "delete",
        Validar: function (data) {
            if ("BotonBorrarValidar" in valores) {
                return valores.BotonBorrarValidar(data);
            }
            return true;
        },
        OnClick: function (data) {
            valores.CallbackBorrar(data, function () {
...
jQuery.fn.DataTableInformacionOrganicaSecretaria = function (valores) {
    ...
    var cols = [
      {
          ...
          render: function (data, type, row) {
              return (
                '<div><span class="link-detalle tooltipped" data-position="bottom" data-delay="50" data-tooltip="Ver detalle" id-entity="' +
                row.Id +
                '">' +
                toTitleCase(data) +
                "</span></div>"
    ...
    function buscarDatos(id) {
        return new Promise(function (callback, callbackError) {
            crearAjax({
                Url: ResolveUrl(
                  "~/Servicios/InformacionOrganicaSecretariaService.asmx/GetById"
                ),
                Data: { id: id },
                OnSuccess: function (result) {
                    if (!result.Ok) {
...
jQuery.fn.DataTableInformacionOrganicaDireccion = function (valores) {
    ...
    var cols = [
      ...
      },
      {
          ...
          render: function (data, type, row) {
              return (
                "<div><span>" + toTitleCase(row.Secretaria.Nombre) + "</span></div>"
              );
          }
      },
      {
    ...
    function verDetalle(idEntity) {
        ...
        crearDialogoInformacionOrganicaDireccionDetalle({
            ...
            Callback: function () {
                buscarDatos(idEntity).then(function (result) {
                    actualizarFila(result);
                });
            }
        });
    ...
    function buscarDatos(id) {
        return new Promise(function (callback, callbackError) {
            crearAjax({
                Url: ResolveUrl(
                  "~/Servicios/InformacionOrganicaDireccionService.asmx/GetById"
                ),
                Data: { id: id },
                OnSuccess: function (result) {
                    if (!result.Ok) {
...
jQuery.fn.DataTableInformacionOrganica = function (valores) {
      ...
      },
      {
          ...
          render: function (data, type, row) {
              if (row.InformacionOrganica != undefined) {
                  return (
                    "<div><span>" +
                    toTitleCase(row.InformacionOrganica.Direccion.Secretaria.Nombre) +
                    "</span></div>"
                  );
              } else {
                  return "<div><span></span></div>";
      ...
      },
      {
          ...
          render: function (data, type, row) {
              if (row.InformacionOrganica != undefined) {
                  return (
                    "<div><span>" +
                    toTitleCase(row.InformacionOrganica.Direccion.Nombre) +
                    "</span></div>"
                  );
              } else {
                  return "<div><span></span></div>";
    ...
    function verDetalle(idArea) {
        ...
        crearDialogoInformacionOrganicaDetalle({
            ...
            Callback: function () {
                buscarDatos(idArea).then(function (result) {
    ...
    function buscarDatos(id) {
        return new Promise(function (callback, callbackError) {
            crearAjax({
                Url: ResolveUrl(
                  "~/Servicios/InformacionOrganicaService.asmx/GetByIdArea"
                ),
                Data: { idArea: id },
                OnSuccess: function (result) {
                    if (!result.Ok) {
...
function procesarDatatable(idTabla, valores) {
            ...
            render: function (data, type, row) {
                ...
                $.each(valores.Botones, function (index, btn) {
                    ...
                    if ("MostrarTooltip" in valores && valores.MostrarTooltip == false) {
                        claseBoton =
                          "btn btn-cuadrado chico no-select btnTabla waves-effect";
                    } else {
                        claseBoton =
                          "btn btn-cuadrado chico tooltipped no-select btnTabla waves-effect";
                    }
                ...
                if (algunInvisible) {
                    var html_boton = $("<a/>", {
                        class:
                          "btn btn-cuadrado chico tooltipped no-select btnMenu waves-effect",
                        "data-position": "bottom",
    ...
    if ("InputBusqueda" in valores) {
        $(valores.InputBusqueda).on("input", function () {
            dt.search(
              getInputs(valores.InputBusqueda, valores.InputBusquedaSelect)
            ).draw(false);
        });
        $("#" + idTabla)
    ...
    if ("InputBusquedaSelect" in valores) {
        $(valores.InputBusquedaSelect).on("change", function () {
            dt.search(
              getInputs(valores.InputBusqueda, valores.InputBusquedaSelect)
            ).draw(false);
        });
    ...
    function getInputs(inputBusqueda, inputBusquedaSelect) {
        var texto = "";
        ...
        if (inputBusquedaSelect == undefined) return texto;

        var valores = $(inputBusquedaSelect)[0].selectedOptions;
...
function moverDatatableFooter(selectorTabla, selectorDestino) {
    ...
    $(selectorTabla + "_wrapper")
      .find(".dataTables_info")
      .detach()
      .appendTo($(selectorDestino));
    $(selectorTabla + "_wrapper")
